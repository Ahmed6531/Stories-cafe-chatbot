...
router.post("/transcribe", upload.single("audio"), async (req, res) => {
  try {
    ...
    // Bilingual prompt — Whisper auto-detects the language; the vocab anchors it
    // to menu terms in both English and Arabic so short clips aren't misclassified.
    const prompt = process.env.STT_PROMPT ||
      "latte cappuccino americano espresso mocha macchiato frap frappuccino " +
      "flat white matcha chai oat milk almond milk large medium small iced hot decaf " +
      "قهوة لاتيه كابتشينو أمريكانو فراب ماكياتو إسبريسو موكا حليب شوفان حليب لوز كبير وسط صغير مثلج ساخن";
