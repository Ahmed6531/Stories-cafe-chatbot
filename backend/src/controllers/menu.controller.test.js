...
describe('Menu Controller CRUD Operations', () => {
  ...
  describe('createMenuItem', () => {
    it('creates a new menu item with all required fields', async () => {
      ...
      MenuItem.findOne.mockResolvedValue(null) // No existing items
    ...
    it('generates correct next numeric ID', async () => {
      ...
      MenuItem.findOne.mockResolvedValue({ id: 42 }) // Mock last item
      MenuItem.prototype.save = jest.fn().mockResolvedValue({
        id: 43
      })
      ...
      // Check that the controller tried to use the correct ID
      expect(res.status).toHaveBeenCalledWith(201)
    })
  ...
  describe('updateMenuItem', () => {
    ...
    it('prevents updating the numeric ID field', async () => {
      ...
      // The id field should have been deleted before the update
      expect(res.status).toHaveBeenCalledWith(200)
    })
    ...
    it('returns 400 when no valid fields are provided', async () => {
      req.params = { id: '1' }
      req.body = { id: 1 } // Only id field, which gets deleted
