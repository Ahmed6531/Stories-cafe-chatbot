...
import ChatBubble from './ChatWidget/ChatBubble'
import ChatInputBar from './ChatWidget/ChatInputBar'
import VoiceMicZone from './ChatWidget/VoiceMicZone'
import { useChatbot } from './ChatWidget/useChatbot'
import { useChatSession, clearChatStorage } from './ChatWidget/useChatSession'
...
export default function Navbar() {
  ...
  const { brand } = theme
  const { messages, setMessages, chipsVisible, setChipsVisible, appendMessage } = useChatSession()
  const {
    typing,
    setTyping,
    voiceActive,
    ...
    handleChipClick,
    pendingCheckoutRef,
  } = useChatbot({ appendMessage, onCheckoutRedirect: closeChat })
  ...
  useEffect(() => {
    ...
    return undefined
  }, [isChatAllowedRoute, chatOpen, chatClosing, chatRouteClosing, stopPendingReply])
  ...
  useEffect(() => {
    ...
    clearChatStorage()
  }, [isSuccessRoute, resetChatbot, setMessages, setChipsVisible])
  const openChat = () => {
    if (!isOnline) return
    if (menuClosing) {
      setMenuClosing(false)
              ...
              <aside className="chat-panel">
                ...
                <section className="chat-conversation" aria-label="Conversation area">
                  ...
                  <VoiceMicZone
                    ...
                    isOnline={isOnline}
                    onMicClick={cycleMicMode}
                    onChipClick={handleChipClick}
                  />
                </section>

                <ChatInputBar value={chatInput} onChange={setChatInput} onSend={handleSend} />
              </aside>
            </div>
          </div>
        )}
