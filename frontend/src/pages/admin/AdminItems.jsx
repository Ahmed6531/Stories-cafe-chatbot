...
const EMPTY_FORM = {
  ...
  description: "",
  isAvailable: true,
  isFeatured: false,   // ✅ exposed so admins can mark homepage items
  slug: "",
}
...
export default function AdminItems() {
  ...
  // ── Image state ──────────────────────────────────────────────────────────────
  // imageFile  — the raw File the user picked (sent to backend)
  // imagePreview — object URL for the thumbnail (revoked on cleanup)
  const [imageFile, setImageFile] = useState(null)
  const [imagePreview, setImagePreview] = useState("")   // current item's saved URL or local preview
  ...
  function validateForm() {
    ...
    // Image is required only on create when no file has been picked
  ...
  function startEdit(item) {
    ...
    setForm({
      ...
      isFeatured: item.isFeatured ?? false,   // ✅ populated
      slug: item.slug || "",
    })
    // Show existing image from DB as the preview; no File yet
    resetImage()
  ...
  return (
    <PageWrap>
      ...
      <FormCard component="form" onSubmit={onSubmit}>
        ...
        <FieldInput
          name="slug"
          placeholder="Slug (unique, e.g. cappuccino)"
          value={form.slug}
          onChange={onFormChange}
        />
        ...
        {/* ✅ isFeatured — controls homepage featured section */}
        <CheckRow>
          <input
            type="checkbox"
            name="isFeatured"
          ...
          <tbody>
            {items.map((item) => (
              <tr key={item.id}>
                <Td>{item.name}</Td>
                <Td>{item.category}</Td>
                <Td sx={{ fontSize: 12, color: "text.secondary" }}>{item.slug}</Td>
                <Td>
                  {item.image ? (
                    <img
                      src={item.image}
                ...
                <Td>
                  <Badge $yes={item.isAvailable}>{item.isAvailable ? "Yes" : "No"}</Badge>
                </Td>
                {/* ✅ Featured column */}
                <Td>
                  <Badge $yes={item.isFeatured}>{item.isFeatured ? "Yes" : "No"}</Badge>
                </Td>
                <Td>
                  <Box sx={{ display: "flex", gap: 1 }}>
                    ...
                    <DangerBtn type="button" onClick={() => handleDelete(item.id)}>
                      Delete
                    </DangerBtn>
                  </Box>
                </Td>
