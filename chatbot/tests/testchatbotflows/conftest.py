...
def fake_session(session_id: str = "test-session", stage: str | None = None) -> dict:
    ...
    Includes guided_order_* fields present in the current session_store implementation.
    """
    return {
...
def mock_llm_response(intent: str, items: list[dict] | None = None) -> dict:
    """Returns a structured dict matching what resolve_intent returns.

    The orchestrator now calls resolve_intent (not try_interpret_message directly),
    so this fixture must match the full resolved intent object.
    """
    is_unknown = intent == "unknown"
    return {
        "intent": intent,
        "confidence": 0.2 if is_unknown else 0.95,
        "items": items or [],
        "follow_up_ref": None,
        "needs_clarification": False,
        "reason": "",
        "source": "llm",
        "route_to_fallback": is_unknown,
        "fallback_needed": is_unknown,
    }
